 ...
 *
 *  Fase 0 – attende {@code startDelayMs} (spaziatura visiva tra minion).
 *  Fase 1 – sposta il JLabel da X_START a X_TORRE (posizione 0→100).
 *            Tempi di traversata con SPEED_MULT = 300:
 *              Mago     (vel 2.8) → ~107 ms/step → ~11 s
 *              Campione (vel 2.2) → ~136 ms/step → ~14 s
 *              Soldato  (vel 2.0) → 150  ms/step → ~15 s
 *              Cannone  (vel 1.5) → 200  ms/step → ~20 s
 *  Fase 2 – attacca la torre ogni secondo.
 *
...
public class MinionThreads implements Runnable {
    ...
    public void run() {
        ...
        while (attivo && minion.isVivo() && minion.getPosizione() < 100) {
