...
/**
 * Thread che gestisce il ciclo di vita grafico e combattivo di un singolo
 * {@link Minion}.
 *
 * Il thread esegue tre fasi sequenziali:
 *
 * Fase 0 – Staggering: attende {@code startDelayMs} millisecondi prima di
 * comparire sullo schermo. Ogni minion dell'ondata parte con 1500 ms di ritardo
 * rispetto al precedente, creando una distanza visiva leggibile. Fase 1 –
 * Avanzamento: sposta il {@link javax.swing.JLabel} associato da sinistra
 * ({@link #X_START}) verso la torre ({@link #X_TORRE}), step per step
 * (posizione 0 → 95). Il delay tra uno step e il successivo vale
 * {@code SPEED_MULT / velocita} ms. Fase 2 – Attacco: il minion è davanti alla
 * torre e la colpisce ogni 1000 ms finché la torre o il minion stesso non
 * muoiono.
 *
 *
...
public class MinionThreads implements Runnable {
    ...
    /**
     * Coordinata X di schermo corrispondente alla posizione logica 0
     * (partenza).
     */
    public static final int X_START = 0;

    /**
     * Coordinata X di schermo corrispondente alla posizione logica 95 (davanti
     * alla torre).
     */
     ...
     * {@code delayStep = SPEED_MULT / velocita} ms. Con velocità 2.0 → 150
     * ms/step → circa 14 s per attraversare la corsia.
     */
    ...
    /**
     * Coordinata Y fissa del label nella corsia. Viene prelevata una sola volta
     * dal form per evitare letture ripetute sull'EDT.
     */
    ...
    /**
     * Flag di controllo del loop principale. Impostato a {@code false} da
     * {@link #ferma()} per interrompere il thread in modo cooperativo a fine
     * ondata o a torre distrutta.
     */
    ...
    /**
     * Segnala al thread di fermarsi al prossimo controllo del flag
     * {@code attivo}. Chiamato da {@link Frm_Gara} a fine ondata o quando la
     * torre viene distrutta.
     */
    public void ferma() {
        attivo = false;
    }
    ...
    /**
     * Esegue le tre fasi: staggering → avanzamento → attacco. Al termine (per
     * morte del minion, torre distrutta o chiamata a {@link #ferma()}) nasconde
     * il label sull'EDT.
     */
    @Override
    public void run() {
