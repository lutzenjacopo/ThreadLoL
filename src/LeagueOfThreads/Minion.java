 ...
 *
 * Ogni minion (e campione) possiede:
 *   posizione – valore 0–100 che indica quanta strada ha percorso nella corsia
 *   vita / vitaMax– punti vita attuali e massimi
 *   attacco – danno inflitto ad ogni colpo
 *   velocita – determina la frequenza degli step di avanzamento in {@link MinionThreads}
 *   tipo – usato da {@link TorreThreads} per la priorità di bersaglio:
 *       SOLDATO &gt; MAGO &gt; CANNONE &gt; CAMPIONE
 * 
 *
 * Il metodo {@link #subisciDanno(double)} è {@code synchronized} per garantire
 * la correttezza quando più thread accedono contemporaneamente alla stessa istanza.
 */
public abstract class Minion {

    /**
     * Tipo del minion, usato da {@link TorreThreads} per scegliere il bersaglio.
     * Ordine di priorità: SOLDATO → MAGO → CANNONE → CAMPIONE.
     */
    ...
    /** Danno inflitto alla torre ad ogni tick di attacco (Fase 2 di {@link MinionThreads}). */
    protected double attacco;

    /**
     * Velocità di avanzamento: più alto = delay per step più breve.
     * Formula in {@link MinionThreads}: {@code delayStep = SPEED_MULT / velocita} ms.
     */
    ...
    /** {@code true} se il minion ha raggiunto la torre e sta attaccando (Fase 2). */
    ...
    /**
     * Ripristina il minion alle condizioni iniziali.
     * Usato tra un'ondata e la successiva se si vuole riutilizzare l'istanza.
     */
    public void reset() {
        this.vita      = vitaMax;
        this.posizione = 0;
    ...
    /**
     * Applica il danno ricevuto, riducendo i punti vita.
     * Se la vita scende a zero (o sotto) il minion viene dichiarato morto.
     *
     * Metodo {@code synchronized} per gestire attacchi concorrenti
     * da {@link TorreThreads} su istanze condivise.
     *
